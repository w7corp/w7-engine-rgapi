...
class bfb_sdk {
     ...
     */
    public function create_baifubao_pay_order_url($params, $url) {
        ...
        if (!in_array(
            ...
            $this->log(
                sprintf(
                        'invalid url[%s], bfb just provide three kind of pay url',
                        $url
                    )
            );
            return false;
        ...
        $this->log(
            sprintf(
                    'the params that create baifubao pay order is [%s]',
                    $params_str
                )
        );
     ...
     */
    public function check_bfb_pay_result_notify() {
        ...
        if (empty($_GET) || !isset($_GET ['sp_no']) || !isset(
            ...
            $this->log(
                sprintf(
                        'missing the params of return_url page, params[%s]',
                        print_r($_GET)
                    )
            );
        }
        ...
        if (sp_conf::SP_PAY_RESULT_WAITING == $order_state) {
            ...
            $this->log(
                sprintf(
                        'the order state is wrong, it is [%s]',
                        $order_state
                    )
            );
     ...
     */
    public function query_baifubao_pay_result_by_order_no($order_no) {
        $params = array(
        ...
        $this->log(
            sprintf(
                    'the url of query baifubao pay result is [%s]',
                    $query_url
                )
        );
        ...
        $this->log(
            sprintf(
                    'the result from baifubao query pay result is [%s]',
                    $content
                )
        );
        ...
        if (empty($response_arr) || !isset($response_arr ['query_status']) ||
            ...
            $this->err_msg = sprintf(
                '百付宝的订单查询接口查询失败，返回数据为[%s]',
        ...
        $response_arr ['goods_name'] = iconv(
            "UTF-8",
            "GBK",
            $response_arr ['goods_name']
        );
        if (isset($response_arr ['buyer_sp_username'])) {
            $response_arr ['buyer_sp_username'] = iconv(
                "UTF-8",
                "GBK",
        ...
        if (is_array($params)) {
            ...
            if (ksort($params)) {
                ...
                if ($params ['sign_method'] == sp_conf::SIGN_METHOD_MD5) {
                    return md5($sign_str);
                } elseif ($params ['sign_method'] == sp_conf::SIGN_METHOD_SHA1) {
